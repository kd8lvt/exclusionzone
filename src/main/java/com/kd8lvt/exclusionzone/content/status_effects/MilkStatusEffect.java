package com.kd8lvt.exclusionzone.content.status_effects;
...
public class MilkStatusEffect extends StatusEffect {
    ...
    private static void lowerTime(LivingEntity target,int amount) {
        StatusEffectInstance self = Objects.requireNonNull(target.getStatusEffect(ModStatusEffects.getEntry("milk")));
        target.setStatusEffect(new StatusEffectInstance(ModStatusEffects.getEntry("milk"),self.getDuration()-amount,self.getAmplifier(),self.isAmbient(),self.shouldShowParticles(),self.shouldShowIcon()),target);
    }
}
